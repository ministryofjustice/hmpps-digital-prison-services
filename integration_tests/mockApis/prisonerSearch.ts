...
const prisonerMockDataA: Prisoner = {
...
export default {
  stubPrisonerSearchPing: (httpStatus = 200) =>
    stubFor({
      request: {
        method: 'GET',
  ...
  stubLocationSearch: ({ prisonId }: { prisonId: string }) =>
    stubFor({
      request: {
        method: 'GET',
        urlPattern: `/prisoner-search/prison/${prisonId}/prisoners.*`,
      },
      response: {
        status: 200,
        headers: { 'Content-Type': 'application/json;charset=UTF-8' },
        jsonBody: {
          content: [prisonerMockDataA],
          pageable: {
            pageNumber: 0,
            pageSize: 100,
            sort: {
  ...
  stubGlobalSearch: () =>
    stubFor({
      request: {
        method: 'POST',
        urlPattern: `/prisoner-search/global-search.*`,
      },
      response: {
        status: 200,
        headers: { 'Content-Type': 'application/json;charset=UTF-8' },
